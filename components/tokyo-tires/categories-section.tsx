...
export function CategoriesSection() {
  ...
  return (
    <section id="categories" ref={ref} className="py-24 md:py-32 bg-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          <motion.span 
            className="text-[#FF2D2D] font-sans text-sm font-bold uppercase tracking-[0.2em]"
            animate={{ 
              opacity: [0.6, 1, 0.6],
              textShadow: [
          ...
          <h2 className="font-display text-5xl md:text-7xl font-bold mt-4 mb-6 text-balance text-gray-900 uppercase tracking-tighter">
            Catálogo <span className="text-[#FF2D2D]">Premium</span>
          </h2>
          <p className="font-sans text-gray-500 max-w-2xl mx-auto text-base tracking-normal">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {categories.map((category, index) => {
            ...
            <motion.div
              ...
              className="group relative bg-white rounded-3xl overflow-hidden cursor-pointer border border-gray-100 hover:border-[#FF2D2D]/30 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.05)] hover:shadow-[0_20px_60px_-10px_rgba(0,0,0,0.1)] transition-all duration-500"
            >
              {/* Neon Breathing Border */}
              <motion.div
                className="absolute inset-0 z-30 pointer-events-none rounded-3xl border-2"
              ...
              {/* Image */}
              <div className="relative h-64 overflow-hidden bg-white flex items-center justify-center p-10">
                {/* Glass Platform */}
                <div className="absolute bottom-4 w-3/4 h-14 bg-gray-50/50 backdrop-blur-sm rounded-[50%] skew-x-[-15deg] border border-gray-200/50 z-0 shadow-inner overflow-hidden">
                   {/* Subtle Reflection */}
              ...
              {/* Content */}
              <div className="relative p-6 bg-white border-t border-gray-50">
                <motion.h3 
                  className="font-display text-xl font-bold mb-2 flex items-center gap-3 transition-all duration-300 text-gray-800 group-hover:text-[#FF2D2D]"
                  whileHover={{ 
                    x: 5,
                ...
                <p className="font-sans text-sm text-gray-500 mb-4 leading-relaxed line-clamp-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {category.specs.map((spec, i) => (
                    <motion.span
                      key={spec}
                      className="flex items-center gap-2 px-3 py-1.5 bg-gray-50 border border-gray-200 text-gray-600 text-[11px] font-medium rounded-full transition-all duration-300"
                      whileHover={{ 
